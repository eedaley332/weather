...
let handleWeatherResponse = function(response) {
  ...
  document.getElementById("current-conditions-text").innerHTML = "Currently " + Math.round(response.currently.temperature) + " and " + response.currently.summary;

  document.getElementById("current-conditions-icon").innerHTML = icon(response.currently);

  $(".forecast").empty();

  let heading = "6 Day Forecast:";
